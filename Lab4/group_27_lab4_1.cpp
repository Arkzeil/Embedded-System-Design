...
int main(int argc, const char *argv[])
{
	...
	std::cout << "camera: " << frame_width <<'x' << frame_height << std::endl;

	x_offset = (framebuffer_width - frame_width) / 2;
    	y_offset = (framebuffer_height - frame_height) / 2;
	...
	while (1){
		...
		// http://www.cplusplus.com/reference/ostream/ostream/seekp/
			...
			//ofs.seekp((y+y_offset)*frame_width*2 + x_offset*2);
			//ofs.seekp(y*frame_width*2 + x_offset + y_offset - frame_width);
			ofs.seekp((y+y_offset)*framebuffer_width*2 + x_offset*2);
		...
		if(flag){ // save screenshot after input
			//sprintf(img_path, "/root/lab3/screenshot/screenshot%d.png", count);
			sprintf(img_path, "/run/media/sda1/screenshot/screenshot%d.png", count++);
			//std::cout << img_path << std::endl;
			cv::imwrite(img_path, ini_frame);
			//std::cout << count++ << std::endl; //this can commented for better performance
			flag = false; //release flag
		}
		
		//video.write(ini_frame);
...
struct framebuffer_info get_framebuffer_info(const char *framebuffer_device_path)
{
    ...
    // fb_info.bits_per_pixel =     // 16
		...
		//x_offset = (screenWidth - imageWidth) / 2;
    		//y_offset = (screenHeight - imageHeight) / 2;
